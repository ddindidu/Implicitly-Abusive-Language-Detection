...
def main(args):
    ...
    data_path = './analysis/results/preds_depending_on_thres'
    df_pred = pd.DataFrame(all_preds)
    col_name = 'preds_{}'.format(args.threshold)
    df_pred.columns = [[col_name]]
    file_name = '{}_{}_{}.csv'.format(args.data_type, args.task_type, args.threshold)
    df_pred.to_csv(data_path+'/'+file_name, index=False)
